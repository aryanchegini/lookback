...
class Lookback:
    ...
    def backtest(self, strategy: Strategy, prices: pd.DataFrame, *,
                 cost_bps: float = 0.0, borrow_bps: float = 0.0,
                 sizer: Sizer | None = None) -> BacktestResult:
        return VectorisedBacktester(
            cost_bps=cost_bps, borrow_bps=borrow_bps, sizer=sizer
        ).run(strategy, prices)

    def portfolio(self, strategy: Strategy,
    ...
    def sweep(self, build, prices: pd.DataFrame, *, cost_bps: float = 0.0,
              rank_by: str = "sharpe", **axes) -> pd.DataFrame:
        return run_sweep(build, prices, param_grid(**axes),
                         cost_bps=cost_bps, rank_by=rank_by)
